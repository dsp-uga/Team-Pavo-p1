...
if __name__ == "__main__":
    ...
    stopWords = np.loadtxt(STOP_FILE, dtype = np.str).tolist()
    #PREPROCESS
    if CLASSIFIER == BAY:
        '''
        ...
        #filter out the word only appear once in all the documents
        hitword=X.map(lambda x:(x[1],1)).reduceByKey(add).filter(lambda x: x[1]>1).keys().collect()
        X=X.filter(lambda x: x[1] in hitword )
